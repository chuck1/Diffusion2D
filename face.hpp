...
class Face: public LocalCoor {
	Face(Patch* patch, int normal, array<real,2> const & ext, real pos_z, array<int,1> n): LocalCoor(normal) {
		...
		/*ARR_LOOP_START(d_) {
			...
			ARR_LOOP(d_, p0, 1) {
				//for(int k = 0; k < 2; ++k) {
				ARR_LOOP(d_, p1, 2) {
					*p2 = (ext_.get(k,1) - ext_->get(k,0)) / ((float)n_.get(k));
				}
			}
		}*/
	...
	Index_Lambda	index_lambda(Face* nbr) {
		// returns lambda:
		...
		int PG = nbr->get_loc_pos_par_index(this);
	...
	void		send_array(Equation* equ, Conn* conn) {
		auto v = make_ones<real,1>({n_->get(conn->pl_)});
	...
	Term		term(Equation* equ, int ind[2], int v, int sv, real To) {
	...
	void		step_pre_cell(Equation* equ, std::vector<int> ind, int V) {
		// set the v-array value for the boundary value at ind+V
		IS v(V);
		
		Conn* conn = loc_to_conn(V);
		if(conn) {
	...
	void		step_pre_cell_open_bou(Equation* equ, std::vector<int> ind, int V) {
